    ...
    <div id="jurisdicciones" style="overflow-x:auto;">
        ...
        <table class="table table-borderless table-striped border" v-model="organismos">
            ...
            <tr style="text-align: center;" v-for="organismo in organismos">
                ...
                <td><a data-toggle="modal" data-target="#mostrarModal" @click="mostrarJurisdiccion(organismo)" >{{ organismo.organismo }}</a></td>
                <td>{{ organismo.jurisdiccion_id }}</td>
                <td>{{ organismo.created_at }}</td>
                <td>{{ organismo.updated_at }}</td>
                <td class="td-button">
                    <a @click="editJurisdiccion(organismo)" class="btn btn-outline-warning border-0  btn-sm shadow" data-toggle="modal" data-target="#editarModal"><i class="far fa-edit"></i></a>
                </td>
                <td class="td-button">
                    <form @submit.prevent="deleteJurisdiccion(organismo.id)">
