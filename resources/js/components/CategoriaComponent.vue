<template>
  ...
  <div id="lista_categorias">
    ...
    <div class="modal fade" id="categoria_new" tabindex="-1" role="dialog" aria-labelledby="ModalLabelNewCategoria" aria-hidden="true">
          ...
          <form  action="" @submit.prevent="nuevaCategoria()">
            ...
            <div class="modal-body">
              <div class="form-group row">
                <label for="input_codigo_categoria" class="col-sm-3 col-form-label">Codigo *</label>
                <div class="col-sm-9">
                  <input type="text" class="form-control" id="input_codigo_categoria" v-model="cod_categoria" >
                  <span class="errors text-danger" v-for="error in errores.cod_categoria">    
                    <small><em>{{error}}</em></small>
                  </span>
                </div>
              </div>
              <div class="form-group row">
                <label for="input_categoria" class="col-sm-3 col-form-label">Categoria *</label>
                <div class="col-sm-9">
                  <input type="text" class="form-control" id="input_categoria" v-model="descripcion" >
                  <span class="errors text-danger" v-for="error in errores.categoria">
                      <small><em>{{error}}</em></small>
                  </span>
                </div>
              </div>
              <div class="form-group row">
                <label for="select_categoria" class="col-sm-3 col-form-label">Jurisdicción *</label>
                <div class="col-sm-9">
                  <select :disabled="jurisdicciones.length === 0" class="custom-select" id="select_categoria" v-model="selectedJurisdiccion" required>
                  ...
                  <span class="errors text-danger" v-for="error in errores.jurisdiccion_id">
                      <small><em>{{error}}</em></small>
                  </span>
                </div>
              </div>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <!-- {{clase}} -->
            <div class="form-group row">
              <label for="input_codigo_categoria_edit" class="col-sm-3 col-form-label">Codigo *</label>
              <div class="col-sm-9">

                <input v-if='editMode' type="text" class="form-control" id="input_codigo_categoria_edit" placeholder="Codigo" v-model="cod_categoria" disabled>
                <p class="text-justify" v-else>{{cod_categoria}}</p>
                <span class="errors text-danger" v-for="error in errores.cod_categoria">
                    <small><em>{{error}}</em></small>
                </span>
              </div>
            ...
            <div class="form-group row">
              <label for="input_clase_edit" class="col-sm-3 col-form-label">Categoria *</label>
              <div class="col-sm-9">
                <input v-if="editMode" type="text" class="form-control" id="input_clase_edit" placeholder="Categoria" v-model="descripcion">
              ...
              <div class="card">
                ...
                <div class="card-body table-responsive p-0">
                  <table class="table table-hover text-nowrap">
                    <thead>
                      <tr>
                        <th>Cod<a href="#" class="text-dark" @click="sort('cod_categoria')"><small><i class="fas fa-sort"></i></small></a></th>
                        ...
                        <th>Jurisdicción<!-- <a href="#" class="text-dark" @click="sort('categoria')"><small><i class="fas fa-sort"></i></small></a> --></th>
                        <th></th>
                      </tr>
                    </thead>
                    <tbody>
                      ...
                      <tr v-else v-for="(categoria, index) in categorias" :key="categoria.id">
                        ...
                        <td>
                          ...
                          <button class="btn btn-outline-danger rounded-circle btn-xs border-0" @click="eliminarCategoria(index,categoria.id)" ><i class="fas fa-trash"></i></button>
                        </td>
                      </tr>
                    </tbody>
                  </table>
     ...
     export default {
      ...
      methods:{
        ...
        nuevaCategoria(){
            ...
            formData.append('jurisdiccion_id', this.jurisdicciones[this.selectedJurisdiccion].id)
            formData.append('categoria', this.descripcion)
            axios.post('api/categoria/create', formData).then((response)=>{
              console.log(response.data);
              $('#categoria_new').modal('hide');
              Toast.fire({
                icon: 'success',
                title:'Se agrego la nueva categoria '+response.data.categoria,
                background:'#E7FFD7',
              })
        ...
        actualizarCategoria(){
          ...
          axios.put(`api/categoria/update/${this.categoria.id}`,params)
          .then((response)=>{
              Toast.fire({
                icon: 'success',
                title:'La categoria '+response.data.categoria +' se edito con exitó',
                background:'#E7FFD7',
              })
        ...
        eliminarCategoria(index,id){
          ...
          swal.fire({
            title: 'Esta seguro?',
            text: this.categorias[index].categoria+" esta a punto de ser eliminada!",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
              axios.delete(`api/categoria/delete/${id}`).then((response)=>{
